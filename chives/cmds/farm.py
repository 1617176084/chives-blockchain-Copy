...
@farm_cmd.command("uploadfarmerdata", short_help="Upload the farm summary and challenges data to community.chivescoin11111111.org, and you can query the data in this site.")
@click.option(
    "-p",
    "--rpc-port",
    help=(
...
def uploadfarmerdata_cmd(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    while 1==1:
        ...
        print(f"Upload the farm summary and challenges data to community.chivescoin11111111.org, and you can query the data in this site.")
        content = requests.post('https://community.chivescoin11111111.org/farmerinfor/uploaddata.php', data={'FarmerSatus':FarmerSatusJson}).json()
        print(content)
        time.sleep(5)
